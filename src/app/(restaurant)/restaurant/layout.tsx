    ...
    async function loadStaffData() {
      ...
      try {
        ...
        if (user) {
          ...
          // For now, fetch all active outlets (in production, would filter by staff assignment)
          ...
          if (outlets && outlets.length > 0) {
            setManagedOutlets(outlets as Outlet[]);
            // Restore selected outlet from localStorage
            const savedOutletId = localStorage.getItem("pnut_selected_outlet");
            const saved = (outlets as Outlet[]).find((o) => o.id === savedOutletId);
            setSelectedOutlet(saved ?? (outlets as Outlet[])[0]);
            return;
          }
        }
